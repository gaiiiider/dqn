...
class FootballEnv(gym.Env):
    def __init__(self):
        ...
        self.max_score = 4_000
        self.score = 0
        self.DIRECT = {'0': (0, -1), '1': (1, 0), '2': (0, 1), '3': (-1, 0)}
        self.FORCE_MULTIPLIER = 70
        self.MAX_SPEED = 300
    ...
    def render(self):
        ...
        velos_a_text = self.font.render(
            f'velos a: {str(round(self.player_a.velocity.x))}/{str(round(self.player_a.velocity.y))}',
            True, (255, 255, 255))
        self.screen.blit(velos_a_text, (10, 70))

        velos_b_text = self.font.render(
            f'velos b: {str(round(self.player_b.velocity.x))}/{str(round(self.player_b.velocity.y))}',
            True, (255, 255, 255))
